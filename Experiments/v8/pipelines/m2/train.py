...
def main() -> None:
    ...
    logger.log(f'train loader sizes: all={len(dls.train_all)} pretrain_labeled={len(dls.train_pretrain_labeled)} val={len(dls.val_all)} test={len(dls.test_all)}')
    ...
    write_manifest(
        ...
        inputs=input_paths,
        outputs=outputs,
        extra={'run_id': run_id, 'feature_id': feature_id, 'best_epoch': int(res.best_epoch), 'best_score': float(res.best_score), 'thresholds': thresholds, 'task_aliases': task_aliases},
    )
