...
def train_structured_m2(
    *,
    ...
    for epoch in range(1, epochs + 1):
        after_pretrain = max(0, epoch - int(pretrain_supervised_epochs))
        if epoch <= int(pretrain_supervised_epochs):
            beta_kl_epoch = 0.0
            gen_weight_epoch = 0.0
        else:
            if int(kl_warmup_epochs) > 0:
                beta_kl_epoch = float(beta_kl) * min(1.0, float(after_pretrain) / float(int(kl_warmup_epochs)))
            else:
                beta_kl_epoch = float(beta_kl)
            if int(gen_warmup_epochs) > 0:
                gen_weight_epoch = float(gen_weight) * min(1.0, float(after_pretrain) / float(int(gen_warmup_epochs)))
            else:
                gen_weight_epoch = float(gen_weight)

        model.train()
        epoch_sums: Dict[str, float] = {}
        epoch_batches = 0
        use_pretrain_loader = int(pretrain_supervised_epochs) > 0 and epoch <= int(pretrain_supervised_epochs) and train_pretrain_labeled is not None
        epoch_loader = train_pretrain_labeled if use_pretrain_loader else train_all
        loader_name = 'train_pretrain_labeled' if use_pretrain_loader else 'train_all'
        pbar = tqdm(epoch_loader, desc=f'epoch {epoch}/{epochs}', leave=False)
        for batch in pbar:
            loss, info = structured_m2_batch_loss(
                model,
                batch,
                priors=priors,
                device=device,
                beta_kl=beta_kl_epoch,
                lambda_recon=lambda_recon,
                gen_weight=gen_weight_epoch,
                alpha_protox=alpha_protox,
            ...
            if logger:
                logger.log_metrics({'kind': 'batch_train', 'epoch': epoch, **info})
        ...
        if logger:
            logger.log(f'epoch {epoch}: loader={loader_name} beta_kl={beta_kl_epoch:.4f} gen_weight={float(gen_weight_epoch):.4f} free_bits_per_dim={float(free_bits_per_dim):.4f} train={epoch_train} val={val_out.metrics}')
            logger.log_metrics({
                'kind': 'epoch_train',
                'epoch': epoch,
                'loader': loader_name,
                **{f'train_{k}': float(v) for k, v in epoch_train.items()},
            })
            payload = {
                'kind': 'epoch_val',
                'epoch': epoch,
                'beta_kl_epoch': float(beta_kl_epoch),
                'gen_weight_epoch': float(gen_weight_epoch),
                'free_bits_per_dim_epoch': float(free_bits_per_dim),
                'val_score': score,
            }
            for task in binary_tasks:
        ...
        save_checkpoint(run_dir / 'checkpoints' / 'last.pt', model=model, optimizer=opt, epoch=epoch, extra={'val': val_out.metrics, 'score': score})

        if not np.isnan(score) and score > best_score:
            best_score = score
            best_epoch = epoch
            bad = 0
            save_checkpoint(run_dir / 'checkpoints' / 'best.pt', model=model, optimizer=opt, epoch=epoch, extra={'val': val_out.metrics, 'score': score})
        else:
            bad += 1
            if bad >= patience:
                if logger:
    ...
    ckpt_name = 'best.pt' if (run_dir / 'checkpoints' / 'best.pt').exists() else 'last.pt'
    return TrainResult(best_epoch=best_epoch, best_score=best_score, last_epoch=epoch, checkpoint_used=ckpt_name)
