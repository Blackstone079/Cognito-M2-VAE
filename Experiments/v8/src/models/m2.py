...
class IFMMapper(nn.Module):
    """Independent Feature Mapping (IFM) style sinusoidal mapping for molecular features.

    The mapping follows the per-dimension sinusoidal structure described in Xia et al. (NeurIPS 2023):
        f(x) = [sin(v) || cos(v)], v = [2π c_1 x, ..., 2π c_k x]
    where the shared frequencies c_j are learnable and initialized from N(0, σ).
    In this implementation the mapping is applied to fingerprint bits only; descriptor channels are
    concatenated afterwards in raw standardized form.
    """
    ...
    def __init__(self, in_dim: int, num_frequencies: int = 0, *, learnable: bool = True, include_raw: bool = False, init_std: float = 6.0) -> None:
        ...
        if self.num_frequencies > 0:
            c = torch.empty(self.num_frequencies, dtype=torch.float32)
            nn.init.normal_(c, mean=0.0, std=self.init_std)
            self.c = nn.Parameter(c, requires_grad=bool(learnable))
        else:
...
class StructuredM2VAE(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        d_qx = self.ifm.out_dim + dims.d_desc

        self.qy_trunk = mlp(d_qx, clf_hidden, clf_hidden[-1], dropout=dropout)
        h_dim = clf_hidden[-1]
        ...
        self.qz = mlp(d_qx + dims.y_dim, enc_hidden, 2 * dims.z_dim, dropout=dropout)
    ...
    def map_x(self, x_fp: torch.Tensor, x_desc: torch.Tensor) -> torch.Tensor:
    ...
    def q_y_logits(self, x_fp: torch.Tensor, x_desc: torch.Tensor) -> Dict[str, torch.Tensor]:
        h = self.qy_trunk(self.map_x(x_fp, x_desc))
    ...
    def encode_z(self, x_fp: torch.Tensor, x_desc: torch.Tensor, y_struct: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        h = self.qz(torch.cat([self.map_x(x_fp, x_desc), y_struct], dim=1))
