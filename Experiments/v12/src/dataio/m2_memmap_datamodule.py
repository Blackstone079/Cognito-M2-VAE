...
class StructuredMemmapDataset(Dataset):
    def __init__(self, feats: StructuredMemmapFeatures, indices: np.ndarray, *, y_protox_override: np.ndarray | None = None, label_overrides: Dict[str, np.ndarray] | None = None):
        self.f = feats
        self.idx = indices.astype(np.int64)
        self.y_protox_override = None if y_protox_override is None else np.asarray(y_protox_override, dtype=np.int64)
        self.label_overrides = {k: np.asarray(v, dtype=np.int64) for k, v in (label_overrides or {}).items()}
        if self.y_protox_override is not None and len(self.y_protox_override) != len(self.idx):
            raise ValueError('y_protox_override length mismatch')
        for task, arr in self.label_overrides.items():
            if len(arr) != len(self.idx):
                raise ValueError(f'label_overrides[{task}] length mismatch')
    ...
    def __getitem__(self, i: int) -> Dict:
        ...
        y_protox = int(self.y_protox_override[i]) if self.y_protox_override is not None else int(self.f.y_protox[j])
        item = {
            'x_fp': x_fp.float(),
            'x_desc': x_desc.float(),
        ...
        for task in self.f.binary_tasks:
            ...
            yv = int(self.label_overrides[task][i]) if task in self.label_overrides else int(self.f.label_arrays[task][j])
...
@dataclass(frozen=True)
class _MaskedTrainLabels:
    y_protox: np.ndarray
    label_arrays: Dict[str, np.ndarray]
    train_known_mask: np.ndarray


def _sample_keep_mask(rng: np.random.RandomState, known_mask: np.ndarray, frac: float) -> np.ndarray:
    frac = float(frac)
    if frac >= 1.0:
        return known_mask.copy()
    if frac <= 0.0:
        return np.zeros_like(known_mask, dtype=bool)
    keep = np.zeros_like(known_mask, dtype=bool)
    idx = np.where(known_mask)[0]
    if len(idx) == 0:
        return keep
    n_keep = max(1, int(np.floor(frac * len(idx))))
    chosen = rng.choice(idx, size=n_keep, replace=False)
    keep[chosen] = True
    return keep


def _apply_train_label_mask(feats: StructuredMemmapFeatures, tr: np.ndarray, *, train_label_fraction: float = 1.0, train_label_fraction_by_task: dict | None = None, train_label_mask_seed: int = 0, protox_fraction: float | None = None) -> _MaskedTrainLabels:
    rng = np.random.RandomState(int(train_label_mask_seed))
    y_protox_masked = np.asarray(feats.y_protox)[tr].astype(np.int64).copy()
    label_arrays_masked = {task: np.asarray(feats.get_label_array(task))[tr].astype(np.int64).copy() for task in feats.binary_tasks}
    frac_by_task_raw = train_label_fraction_by_task or {}
    frac_by_task = {}
    for task in feats.binary_tasks:
        alias = feats.binary_task_aliases[task]
        frac_by_task[task] = float(frac_by_task_raw.get(task, frac_by_task_raw.get(alias, train_label_fraction)))
    if protox_fraction is None:
        protox_fraction = float(frac_by_task_raw.get(PROTOX_TASK, train_label_fraction))
    protox_known = y_protox_masked >= 0
    protox_keep = _sample_keep_mask(rng, protox_known, float(protox_fraction))
    y_protox_masked[protox_known & ~protox_keep] = -1
    for task in feats.binary_tasks:
        arr = label_arrays_masked[task]
        known = arr >= 0
        keep = _sample_keep_mask(rng, known, frac_by_task[task])
        arr[known & ~keep] = -1
        label_arrays_masked[task] = arr
    train_known_mask = y_protox_masked >= 0
    for task in feats.binary_tasks:
        train_known_mask |= (label_arrays_masked[task] >= 0)
    return _MaskedTrainLabels(y_protox=y_protox_masked, label_arrays=label_arrays_masked, train_known_mask=train_known_mask)

def _build_labeled_task_indices(masked_train: _MaskedTrainLabels, feats: StructuredMemmapFeatures, labeled_local: np.ndarray) -> Dict[str, np.ndarray]:
    task_indices: Dict[str, np.ndarray] = {}
    if len(labeled_local) == 0:
        return task_indices
    for task in feats.binary_tasks:
        alias = feats.binary_task_aliases[task]
        y_lab = masked_train.label_arrays[task][labeled_local]
...
def build_structured_dataloaders_memmap(
    ...
    *,
    ...
    train_label_fraction_by_task: dict | None = None,
    train_label_mask_seed: int = 0,
    train_label_fraction_protox: float | None = None,
    ...
    masked_train = _apply_train_label_mask(
        feats, tr, train_label_fraction=float(train_label_fraction), train_label_fraction_by_task=train_label_fraction_by_task, train_label_mask_seed=int(train_label_mask_seed), protox_fraction=train_label_fraction_protox,
    )

    train_ds = StructuredMemmapDataset(feats, tr, y_protox_override=masked_train.y_protox, label_overrides=masked_train.label_arrays)
    val_ds = StructuredMemmapDataset(feats, va)
    test_ds = StructuredMemmapDataset(feats, te)

    tr_labeled_local = np.where(masked_train.train_known_mask)[0].astype(np.int64)
    local_all = np.arange(len(tr), dtype=np.int64)
    task_indices_all = {}
    for task in feats.binary_tasks:
        alias = feats.binary_task_aliases[task]
        y_tr = masked_train.label_arrays[task]
        task_indices_all[alias] = np.where(y_tr >= 0)[0].astype(np.int64)
    train_all = _make_train_loader(train_ds, batch_size=batch_size, num_workers=num_workers, pin_memory=pin_memory, shuffle=True, indices_all=local_all, task_indices=task_indices_all, train_mix=train_mix, seed=seed)

    labeled_ds = StructuredMemmapDataset(feats, tr[tr_labeled_local], y_protox_override=masked_train.y_protox[tr_labeled_local], label_overrides={task: masked_train.label_arrays[task][tr_labeled_local] for task in feats.binary_tasks})
    labeled_local_all = np.arange(len(tr_labeled_local), dtype=np.int64)
    labeled_task_indices = _build_labeled_task_indices(masked_train, feats, tr_labeled_local)
    train_pretrain_labeled = _make_train_loader(labeled_ds, batch_size=batch_size, num_workers=num_workers, pin_memory=pin_memory, shuffle=True, indices_all=labeled_local_all, task_indices=labeled_task_indices, train_mix=train_mix, seed=seed + 101)
    train_joint_labeled = _make_train_loader(labeled_ds, batch_size=batch_size, num_workers=num_workers, pin_memory=pin_memory, shuffle=True, indices_all=labeled_local_all, task_indices=labeled_task_indices, train_mix=train_mix, seed=seed + 202)
    tr_unlabeled_local = np.where(~masked_train.train_known_mask)[0].astype(np.int64)
    train_unlabeled = None
    if len(tr_unlabeled_local) > 0:
        train_unlabeled = DataLoader(StructuredMemmapDataset(feats, tr[tr_unlabeled_local], y_protox_override=masked_train.y_protox[tr_unlabeled_local], label_overrides={task: masked_train.label_arrays[task][tr_unlabeled_local] for task in feats.binary_tasks}), batch_size=batch_size, shuffle=True, drop_last=False, num_workers=num_workers, pin_memory=pin_memory)
    train_eval_all = DataLoader(StructuredMemmapDataset(feats, tr), batch_size=batch_size, shuffle=False, drop_last=False, num_workers=num_workers, pin_memory=pin_memory)
