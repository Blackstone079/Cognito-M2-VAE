...
def morgan_fp(
    ...
    *,
    ...
    count_transform: str = 'none',
) -> np.ndarray:
    mode = str(mode).strip().lower()
    count_transform = str(count_transform).strip().lower()
    if mode not in {'bit', 'count'}:
        raise ValueError(f'unknown Morgan fingerprint mode: {mode}')
    if count_transform not in {'none', 'log1p'}:
        raise ValueError(f'unknown Morgan count transform: {count_transform}')
    ...
    if mode == 'count':
        ...
        return fp.astype(np.float32, copy=False)
