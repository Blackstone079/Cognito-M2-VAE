...
def _fit_probe_suite(train_data: dict[str, dict[str, np.ndarray]], val_data: dict[str, dict[str, np.ndarray]], test_data: dict[str, dict[str, np.ndarray]], *, seed: int) -> dict:
    ...
    def build_mlp(*, early_stopping: bool):
        return make_pipeline(StandardScaler(), MLPClassifier(hidden_layer_sizes=(128,), activation='relu', alpha=1e-4, learning_rate_init=1e-3, early_stopping=early_stopping, max_iter=250, random_state=seed))
    out = {}
    for task in sorted(train_data.keys()):
        ...
        mlp_early_stopping = bool(len(ytr) >= 20 and int(class_counts.min()) >= 5)
        for name, builder in [('logreg_probe', build_logreg), ('mlp_probe', lambda: build_mlp(early_stopping=mlp_early_stopping))]:
            try:
                clf = builder()
                clf.fit(Xtr, ytr)
                ...
                task_res[name] = {'threshold': thr, 'val': binary_metrics(yv, p_val, threshold=thr), 'test': binary_metrics(yte, p_test, threshold=thr), 'status': 'ok'}
                if name == 'mlp_probe':
                    task_res[name]['early_stopping'] = bool(mlp_early_stopping)
            except Exception as exc:
                task_res[name] = {'status': 'fit_failed', 'error': f'{type(exc).__name__}: {exc}'}
        out[task] = task_res
    return out
...
def main() -> None:
    ...
    diagnostic_ckpts = []
    for candidate in [res.checkpoint_used, 'best_pretrain.pt', 'best_joint.pt']:
        if candidate not in diagnostic_ckpts and (run_dir / 'checkpoints' / candidate).exists():
            diagnostic_ckpts.append(candidate)
        ...
        for diag_ckpt in diagnostic_ckpts:
            ...
            if bool(log_cfg.get('save_capacity_diagnostics', True)):
                representation_summary_by_checkpoint[diag_ckpt] = {
                    'train': summarize_classifier_representations(collect_classifier_representations(model, dls.train_eval_all, device, classifier_ablation='full')),
            ...
            if bool(log_cfg.get('save_representation_probes', True)):
                probe_results_by_checkpoint[diag_ckpt] = _fit_probe_suite(
                    _collect_task_probe_arrays(model, dls.train_eval_all, device, task_aliases, classifier_ablation='full'),
    ...
    eval_json = {
        ...
        'train_val_gap': _train_val_gap(train_out.metrics, val_out.metrics, task_aliases),
