    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if phase == 'pretrain':
            ...
            n_steps = max(len(labeled_loader), len(train_unlabeled) if train_unlabeled is not None else 0) or len(labeled_loader)
            ...
            for _ in pbar:
                ...
                total_loss = loss_lab
                info_unlab = None
                if unlabeled_iter is not None and float(joint_unlabeled_weight) > 0.0:
                    batch_unlab = next(unlabeled_iter)
                    loss_unlab, info_unlab = structured_m2_batch_loss(model, batch_unlab, priors=priors, device=device, beta_kl=beta_kl_epoch, lambda_recon=lambda_recon, gen_weight=gen_weight_epoch, alpha_protox=alpha_protox, alpha_binary=alpha_binary, normalize_recon=normalize_recon, pos_weights=pos_weights, free_bits_per_dim=free_bits_per_dim)
                    total_loss = total_loss + float(joint_unlabeled_weight) * loss_unlab
                opt.zero_grad(set_to_none=True)
                total_loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip)
                opt.step()
