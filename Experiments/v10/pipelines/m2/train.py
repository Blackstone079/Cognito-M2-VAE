...
from src.training.m2_loops import StructuredPriors, evaluate_structured, train_structured_m2  # noqa: E402
...
def main() -> None:
    ...
    dls = build_structured_dataloaders_memmap(
        ...
        num_workers=0,
        pin_memory=False,
        train_mix=cfg['training'].get('batch_mix', {}),
        seed=seed,
    )
    ...
    alpha_binary_default = float(cfg['training'].get('alpha_binary_default', 1.0))
    alpha_binary = {alias: float(cfg['training'].get(f'alpha_{alias}', alpha_binary_default)) for alias in task_aliases}
    score_weights = {alias: float(cfg['training'].get(f'score_weight_{alias}', 1.0)) for alias in task_aliases}
    ...
    res = train_structured_m2(
        ...
        beta_kl=float(cfg['training'].get('beta_kl', 1.0)),
        lambda_recon=float(cfg['training'].get('lambda_recon', 1.0)),
        gen_weight=float(cfg['training'].get('gen_weight', 1.0)),
        ...
        joint_unlabeled_weight=float(cfg['training'].get('joint_unlabeled_weight', 0.25)),
        joint_steps_mode=str(cfg['training'].get('joint_steps_mode', 'labeled')),
        joint_unlabeled_batches_per_step=int(cfg['training'].get('joint_unlabeled_batches_per_step', 1)),
    ...
    thresholds = _choose_thresholds(val_raw, task_aliases, metric=str(cfg['training'].get('threshold_metric', 'bal_acc')))
    ...
    checkpoint_eval = {}
    for ckpt_name in ['best_pretrain.pt', 'best_joint.pt', 'best_overall.pt', 'last.pt']:
        ...
        thresholds_ckpt = _choose_thresholds(val_raw_ckpt, task_aliases, metric=str(cfg['training'].get('threshold_metric', 'bal_acc')))
        checkpoint_eval[ckpt_name] = {
            'thresholds': thresholds_ckpt,
            'train': evaluate_structured(model, dls.train_eval_all, device, priors, thresholds=thresholds_ckpt).metrics,
            'val': evaluate_structured(model, dls.val_all, device, priors, thresholds=thresholds_ckpt).metrics,
            'test': evaluate_structured(model, dls.test_all, device, priors, thresholds=thresholds_ckpt).metrics,
        }
    load_checkpoint(ckpt_path, model=model)
    model.to(device)
