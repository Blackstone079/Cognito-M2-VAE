...
from src.features.featurize_rdkit import DescScaler, desc8, mol_from_smiles, morgan_fp  # noqa: E402
...
def main() -> None:
    ...
    label_arrays = {
        task: np.lib.format.open_memmap(out_dir / binary_files[task], mode='w+', dtype=np.int64, shape=(N,))
    ...
    with split_summary_csv.open('w', newline='', encoding='utf-8') as f:
        ...
        for sp in ('train', 'val', 'test'):
            row = {
                'split': sp,
                'total': int(sum(counts['protox'][sp].values())),
                'protox_labeled': int(sum(v for k, v in counts['protox'][sp].items() if int(k) != UNKNOWN)),
            }
            for task in binary_tasks:
                alias = binary_aliases[task]
    ...
    output_paths = [
        out_dir / 'X_fp.npy',
        out_dir / 'X_desc.npy',
        out_dir / 'y_protox.npy',
        out_dir / 'split_code.npy',
        out_dir / 'inchi_key.npy',
        meta_path,
        scaler_path,
        split_summary_csv,
        split_label_counts_csv,
        scaffold_meta_csv,
    ]
    ...
    write_manifest(
        ...
        stage_name='02_build_features',
        config_path=cfg_path,
        inputs=[in_csv, split_path],
