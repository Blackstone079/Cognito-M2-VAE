...
class TrainResult:
    ...
    best_equalweight_epoch: int = -1
    best_equalweight_score: float = float('nan')
    pretrain_epochs_used: int = 0
    joint_unlabeled_batches_seen: int = 0
    grad_clip_hits: int = 0
    grad_clip_total_batches: int = 0
...
def _fp_recon_loglik(model: StructuredM2VAE, x: torch.Tensor, fp_param: torch.Tensor, fp_logvar: torch.Tensor | None, *, normalize: bool) -> torch.Tensor:
    if model.fp_recon_distribution == 'bernoulli':
        return _bernoulli_loglik_with_logits(x, fp_param, normalize=normalize)
    if fp_logvar is None:
        raise ValueError('Gaussian fingerprint reconstruction requires fp_logvar.')
    return _gaussian_loglik(x, fp_param, fp_logvar, normalize=normalize)
...
def _diag_gaussian_logprob(x: torch.Tensor, mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    two_pi = torch.tensor(2.0 * np.pi, device=x.device, dtype=x.dtype)
    return -0.5 * torch.sum(logvar + (x - mu).pow(2) / torch.exp(logvar) + torch.log(two_pi), dim=1)


def _standard_normal_logprob(x: torch.Tensor) -> torch.Tensor:
    two_pi = torch.tensor(2.0 * np.pi, device=x.device, dtype=x.dtype)
    return -0.5 * torch.sum(x.pow(2) + torch.log(two_pi), dim=1)
...
def structured_m2_batch_loss(
    ...
    *,
    ...
    mu, logvar, flow_context = model.encode_z_params(x_fp_rep, x_desc_rep, y_struct_rep)
    z0 = reparameterize(mu, logvar)
    z, flow_log_det = model.apply_flow(z0, flow_context)
    fp_param, fp_logvar, desc_mu, desc_logvar = model.decode_x(z, y_struct_rep)
    ll_fp = _fp_recon_loglik(model, x_fp_rep, fp_param, fp_logvar, normalize=normalize_recon)
    ll_desc = _gaussian_loglik(x_desc_rep, desc_mu, desc_logvar, normalize=normalize_recon)
    if model.uses_iaf:
        kl_scalar = (_diag_gaussian_logprob(z0, mu, logvar) - flow_log_det - _standard_normal_logprob(z)).reshape(B, C)
        kl_per_dim = kl_scalar.unsqueeze(-1).expand(B, C, model.dims.z_dim) / float(model.dims.z_dim)
    else:
    ...
    if not return_details:
        return loss, info
    detail_tensors: Dict[str, torch.Tensor] = {
        'loss_total': loss,
        'loss_unsup': float(gen_weight) * gen_loss,
        'loss_recon_fp': -ll_fp.reshape(B, C).mean(),
        'loss_recon_desc': -ll_desc.reshape(B, C).mean(),
        'loss_kl_obj': beta_kl * torch.sum(kl_obj_per_dim),
        'loss_entropy': torch.mean(entropy),
    }
    for task_name, task_loss in sup.items():
        detail_tensors[f'loss_sup_{task_name}'] = task_loss
    return loss, info, detail_tensors
...
def evaluate_structured(
    ...
    *,
    ...
    for batch in dl:
        x_fp = batch['x_fp'].to(device)
        x_desc = batch['x_desc'].to(device)
        logits = model.q_y_logits(x_fp, x_desc, classifier_ablation=classifier_ablation)
...
def collect_classifier_representations(
    model: StructuredM2VAE,
    dl: DataLoader,
    device: str,
    *,
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            for name, tensor in details.items():
                ...
                if arr.ndim == 1:
                    arr = arr[:, None]
                store.setdefault(name, []).append(arr)
    return {name: np.concatenate(parts, axis=0) for name, parts in store.items() if parts}


def _build_gradient_probe(model: StructuredM2VAE, detail_tensors: dict[str, torch.Tensor]) -> dict[str, dict[str, float]]:
    param_groups = model.diagnostic_parameter_groups()
    scalar_map: dict[str, torch.Tensor] = {}
    for key in ('loss_total', 'loss_unsup', 'loss_recon_fp', 'loss_recon_desc', 'loss_kl_obj', 'loss_entropy'):
        if key in detail_tensors:
            scalar_map[key] = detail_tensors[key]
    for task in model.binary_tasks:
        key = f'loss_sup_{task}'
        if key in detail_tensors:
            scalar_map[key] = detail_tensors[key]
    if model.include_protox and 'loss_sup_protox' in detail_tensors:
        scalar_map['loss_sup_protox'] = detail_tensors['loss_sup_protox']
    ...
    for group_name, params in param_groups.items():
        for p in params:
            ...
            pid = id(p)
            if pid in seen:
                param_group_names[seen[pid]].append(group_name)
            else:
                seen[pid] = len(flat_params)
                flat_params.append(p)
                param_group_names.append([group_name])

    report = {}
    for loss_name, loss_tensor in scalar_map.items():
        if not isinstance(loss_tensor, torch.Tensor) or not loss_tensor.requires_grad:
            report[loss_name] = {group_name: 0.0 for group_name in param_groups.keys()}
            continue
        grads = torch.autograd.grad(loss_tensor, flat_params, retain_graph=True, allow_unused=True)
        group_sums = {name: 0.0 for name in param_groups.keys()}
        for g, groups in zip(grads, param_group_names):
            if g is None:
                continue
            norm_sq = float(torch.sum(g.detach() * g.detach()).cpu())
            for group_name in groups:
                group_sums[group_name] += norm_sq
        report[loss_name] = {group_name: float(np.sqrt(val)) for group_name, val in group_sums.items()}
    return report


def _activation_dead_fraction(arr: np.ndarray) -> float:
    if arr.size == 0:
        return float('nan')
    return float(np.mean(np.mean(np.abs(arr) <= 1e-8, axis=0)))


def _effective_rank(arr: np.ndarray) -> float:
...
def summarize_classifier_representations(reps: dict[str, np.ndarray]) -> dict[str, dict[str, float | int | list[float]]]:
    out = {}
    for name, arr in reps.items():
        if arr.size == 0:
            out[name] = {'n_rows': 0, 'dim': 0}
            continue
        ...
        summary = {
            ...
            'dead_unit_fraction': _activation_dead_fraction(arr if arr.ndim == 2 else arr[:, None]),
        }
        if arr.ndim == 2:
            summary['effective_rank'] = _effective_rank(arr)
        out[name] = summary
    return out
...
def train_structured_m2(
    *,
    ...
    min_joint_epochs_before_early_stop: int = 0,
    min_joint_unlabeled_passes_before_early_stop: float = 0.0,
    beta_schedule: str = 'cyclical',
    beta_cycle_length: int = 12,
    beta_cycle_ramp_ratio: float = 0.5,
    lagging_inference_steps: int = 0,
    lagging_inference_joint_epochs: int = 0,
    sanity_guard_epoch: int = 2,
    sanity_abort_on_constant_preds: bool = True,
    logger=None,
    diagnostics_dir: Path | None = None,
    collect_gradient_probes: bool = True,
) -> TrainResult:
    model.to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    inference_params = [p for p in model.inference_parameters() if p.requires_grad]
    inference_param_ids = {id(p) for p in inference_params}
    ...
    best_joint_epoch = -1
    best_equalweight_score = -1.0
    best_equalweight_epoch = -1
    bad = 0
    binary_tasks = tuple(model.binary_tasks)
    ...
    persistent_unlabeled_iter = _cycle_iter(train_unlabeled) if train_unlabeled is not None else None
    total_unlabeled_batches = len(train_unlabeled) if train_unlabeled is not None else 0
    required_joint_unlabeled_batches = int(np.ceil(float(min_joint_unlabeled_passes_before_early_stop) * float(total_unlabeled_batches))) if total_unlabeled_batches > 0 else 0
    joint_unlabeled_batches_seen = 0
    grad_clip_hits = 0
    grad_clip_total_batches = 0
    grad_probe_records = []
    ...
    for epoch in range(1, epochs + 1):
        ...
        if phase == 'pretrain':
            ...
            for batch in pbar:
                ...
                loss.backward()
                grad_norm_preclip = float(torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip))
                grad_clip_total_batches += 1
                if grad_norm_preclip > float(grad_clip):
                    grad_clip_hits += 1
                opt.step()
                epoch_batches += 1
            ...
            unlabeled_iter = persistent_unlabeled_iter
            ...
            lagging_steps_active = int(lagging_inference_steps) if (int(lagging_inference_steps) > 0 and joint_epoch <= int(lagging_inference_joint_epochs)) else 0
            for _ in pbar:
                ...
                if unlabeled_iter is not None and float(joint_unlabeled_weight) > 0.0 and n_unlabeled_batches > 0:
                    for _u in range(n_unlabeled_batches):
                        unlab_batches.append(next(unlabeled_iter))
                if lagging_steps_active > 0 and inference_params:
                    for _lag in range(lagging_steps_active):
                        lag_loss_lab, _lag_info_lab = structured_m2_batch_loss(model, batch_lab, priors=priors, device=device, beta_kl=beta_kl_epoch, lambda_recon=lambda_recon, gen_weight=gen_weight_epoch, alpha_protox=alpha_protox, alpha_binary=alpha_binary, normalize_recon=normalize_recon, pos_weights=pos_weights, free_bits_per_dim=free_bits_per_dim)
                        lag_total_loss = lag_loss_lab
                        for batch_unlab in unlab_batches:
                            lag_loss_unlab, _lag_info_unlab = structured_m2_batch_loss(model, batch_unlab, priors=priors, device=device, beta_kl=beta_kl_epoch, lambda_recon=lambda_recon, gen_weight=gen_weight_epoch, alpha_protox=alpha_protox, alpha_binary=alpha_binary, normalize_recon=normalize_recon, pos_weights=pos_weights, free_bits_per_dim=free_bits_per_dim)
                            lag_total_loss = lag_total_loss + (float(joint_unlabeled_weight) / float(max(len(unlab_batches), 1))) * lag_loss_unlab
                        opt.zero_grad(set_to_none=True)
                        lag_total_loss.backward()
                        for param in model.parameters():
                            if id(param) not in inference_param_ids:
                                param.grad = None
                        grad_norm_preclip = float(torch.nn.utils.clip_grad_norm_(inference_params, grad_clip))
                        grad_clip_total_batches += 1
                        if grad_norm_preclip > float(grad_clip):
                            grad_clip_hits += 1
                        opt.step()
                if collect_gradient_probes and epoch_batches == 0:
                    loss_lab, info_lab, detail_tensors = structured_m2_batch_loss(model, batch_lab, priors=priors, device=device, beta_kl=beta_kl_epoch, lambda_recon=lambda_recon, gen_weight=gen_weight_epoch, alpha_protox=alpha_protox, alpha_binary=alpha_binary, normalize_recon=normalize_recon, pos_weights=pos_weights, free_bits_per_dim=free_bits_per_dim, return_details=True)
                    grad_probe_records.append({'epoch': epoch, 'phase': phase, 'loader': loader_name, 'probe': _build_gradient_probe(model, detail_tensors)})
                else:
                ...
                if unlab_batches:
                    ...
                    for batch_unlab in unlab_batches:
                        ...
                        total_loss = total_loss + (float(joint_unlabeled_weight) / float(len(unlab_batches))) * loss_unlab
                        joint_unlabeled_batches_seen += 1
                ...
                merged['joint_unlabeled_batches_seen_cum'] = float(joint_unlabeled_batches_seen)
                merged['lagging_inference_steps_active'] = float(lagging_steps_active)
        ...
        epoch_train['grad_clip_hit_rate_cum'] = float(grad_clip_hits / max(grad_clip_total_batches, 1))
        epoch_train['grad_clip_total_batches_cum'] = float(grad_clip_total_batches)
        ...
        score_equal = _score_from_metrics(val_out.metrics, binary_tasks=binary_tasks, score_weights=None)
        ...
        if logger:
            ...
            payload = {'kind': 'epoch_val', 'phase': phase, 'epoch': epoch, 'beta_kl_epoch': float(beta_kl_epoch), 'gen_weight_epoch': float(gen_weight_epoch), 'free_bits_per_dim_epoch': float(free_bits_per_dim), 'val_score': score, 'val_score_equalweight': score_equal, 'joint_unlabeled_batches_seen_cum': float(joint_unlabeled_batches_seen), 'required_joint_unlabeled_batches': float(required_joint_unlabeled_batches)}
            for task in binary_tasks:
        ...
        if not np.isnan(score_equal) and score_equal > best_equalweight_score:
            best_equalweight_score = score_equal
            best_equalweight_epoch = epoch
            save_checkpoint(run_dir / 'checkpoints' / 'best_equalweight.pt', model=model, optimizer=opt, epoch=epoch, extra={'val': val_out.metrics, 'score_equalweight': score_equal, 'phase': phase})
        ...
        if phase == 'pretrain':
            ...
            joint_stop_ready = (joint_epoch >= int(min_joint_epochs_before_early_stop)) and (joint_unlabeled_batches_seen >= int(required_joint_unlabeled_batches))
            if bad >= patience and joint_stop_ready:
                if logger:
                    logger.log(f'early stop at epoch {epoch} (best epoch {best_epoch}, best score {best_score:.4f}); joint_unlabeled_batches_seen={joint_unlabeled_batches_seen}, required_joint_unlabeled_batches={required_joint_unlabeled_batches}')
                break

    if diagnostics_dir is not None and grad_probe_records:
        (diagnostics_dir / 'gradient_probes.json').write_text(json.dumps(grad_probe_records, indent=2), encoding='utf-8')
    ckpt_name = 'best_overall.pt' if (run_dir / 'checkpoints' / 'best_overall.pt').exists() else 'last.pt'
    return TrainResult(best_epoch=best_epoch, best_score=best_score, last_epoch=epoch, checkpoint_used=ckpt_name, best_pretrain_epoch=best_pretrain_epoch, best_pretrain_score=best_pretrain_score if best_pretrain_epoch >= 0 else float('nan'), best_joint_epoch=best_joint_epoch, best_joint_score=best_joint_score if best_joint_epoch >= 0 else float('nan'), best_equalweight_epoch=best_equalweight_epoch, best_equalweight_score=best_equalweight_score if best_equalweight_epoch >= 0 else float('nan'), pretrain_epochs_used=pretrain_epochs_used, joint_unlabeled_batches_seen=joint_unlabeled_batches_seen, grad_clip_hits=grad_clip_hits, grad_clip_total_batches=grad_clip_total_batches)
