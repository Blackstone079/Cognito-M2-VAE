...
from src.training.m2_loops import StructuredPriors, evaluate_structured, train_structured_m2, _score_from_metrics, collect_classifier_representations, summarize_classifier_representations  # noqa: E402
from src.training.m2_metrics import select_binary_threshold, binary_metrics  # noqa: E402
...
def _make_eval_loader_from_dataset(dataset, batch_size: int):
    return DataLoader(dataset, batch_size=int(batch_size), shuffle=False, drop_last=False, num_workers=0, pin_memory=False)


def _metrics_or_empty(out) -> dict:
    return {} if out is None else out.metrics


def _latent_summary_or_empty(out) -> dict:
    return {} if out is None else _latent_summary(out.predictions)
...
def _collect_task_probe_arrays(model, dl, device: str, task_aliases: list[str], *, classifier_ablation: str = 'full') -> dict[str, dict[str, np.ndarray]]:
    store = {task: {'X': [], 'y': []} for task in task_aliases}
    model.eval()
    with torch.no_grad():
        for batch in dl:
            x_fp = batch['x_fp'].to(device)
            x_desc = batch['x_desc'].to(device)
            _logits, details = model.classifier_forward(x_fp, x_desc, classifier_ablation=classifier_ablation, return_details=True)
            rep = details['fusion_rep'].detach().cpu().numpy()
            for task in task_aliases:
                mask = batch[f'mask_{task}'].cpu().numpy() > 0.5
                if not np.any(mask):
                    continue
                store[task]['X'].append(rep[mask])
                store[task]['y'].append(batch[f'y_{task}'].cpu().numpy()[mask].astype(int))
    out = {}
    for task, parts in store.items():
        if parts['X']:
            out[task] = {'X': np.concatenate(parts['X'], axis=0), 'y': np.concatenate(parts['y'], axis=0)}
        else:
            out[task] = {'X': np.empty((0, 0), dtype=float), 'y': np.empty((0,), dtype=int)}
    return out


def _fit_probe_suite(train_data: dict[str, dict[str, np.ndarray]], val_data: dict[str, dict[str, np.ndarray]], test_data: dict[str, dict[str, np.ndarray]], *, seed: int) -> dict:
    def build_logreg():
        return make_pipeline(StandardScaler(), LogisticRegression(max_iter=1000, solver='liblinear', class_weight='balanced', random_state=seed))
    def build_mlp():
        return make_pipeline(StandardScaler(), MLPClassifier(hidden_layer_sizes=(128,), activation='relu', alpha=1e-4, learning_rate_init=1e-3, early_stopping=True, max_iter=250, random_state=seed))
    out = {}
    for task in sorted(train_data.keys()):
        Xtr, ytr = train_data[task]['X'], train_data[task]['y']
        Xv, yv = val_data[task]['X'], val_data[task]['y']
        Xte, yte = test_data[task]['X'], test_data[task]['y']
        if Xtr.size == 0 or Xv.size == 0 or Xte.size == 0 or len(np.unique(ytr)) < 2 or len(np.unique(yv)) < 2 or len(np.unique(yte)) < 2:
            out[task] = {'status': 'insufficient_labeled_data'}
            continue
        task_res = {}
        for name, builder in [('logreg_probe', build_logreg), ('mlp_probe', build_mlp)]:
            clf = builder()
            clf.fit(Xtr, ytr)
            p_val = clf.predict_proba(Xv)[:, 1]
            thr = float(select_binary_threshold(yv, p_val, metric='bal_acc'))
            p_test = clf.predict_proba(Xte)[:, 1]
            task_res[name] = {'threshold': thr, 'val': binary_metrics(yv, p_val, threshold=thr), 'test': binary_metrics(yte, p_test, threshold=thr)}
        out[task] = task_res
    return out


def _available_ablation_modes(model) -> list[str]:
    modes = ['full', 'no_ifm']
    if getattr(model, 'qy_fp_raw_branch', None) is not None:
        modes.append('no_fp_raw')
    if getattr(model, 'qy_desc_branch', None) is not None:
        modes.extend(['no_desc', 'fp_only', 'desc_only'])
    return modes


def _evaluate_ablation_suite(model, dls, device: str, priors, task_aliases: list[str], *, threshold_metric: str) -> dict:
    suite = {}
    for ablation in _available_ablation_modes(model):
        val_raw = evaluate_structured(model, dls.val_all, device, priors, classifier_ablation=ablation)
        thresholds = _choose_thresholds(val_raw, task_aliases, metric=threshold_metric)
        val_out = evaluate_structured(model, dls.val_all, device, priors, thresholds=thresholds, classifier_ablation=ablation)
        test_out = evaluate_structured(model, dls.test_all, device, priors, thresholds=thresholds, classifier_ablation=ablation)
        suite[ablation] = {'thresholds': thresholds, 'val': val_out.metrics, 'test': test_out.metrics}
    return suite


def _train_val_gap(train_metrics: dict, val_metrics: dict, task_aliases: list[str]) -> dict:
    out = {}
    for task in task_aliases:
        out[task] = {}
        for key in ['bal_acc', 'auroc', 'auprc', 'brier', 'ece10']:
            tr = train_metrics.get(task, {}).get(key)
            va = val_metrics.get(task, {}).get(key)
            out[task][key] = None if tr is None or va is None else float(tr - va)
    return out
...
def main() -> None:
    ...
    model = StructuredM2VAE(
        ...
        classifier_fp_view=str(cfg['model'].get('classifier_fp_view', 'binary_presence')),
        classifier_use_descriptor_branch=bool(cfg['model'].get('classifier_use_descriptor_branch', True)),
        classifier_use_logcount_branch=bool(cfg['model'].get('classifier_use_logcount_branch', False)),
        classifier_logcount_branch_dim=cfg['model'].get('classifier_logcount_branch_dim', None),
        iaf_num_flows=int(cfg['model'].get('iaf_num_flows', 0)),
        iaf_hidden_dim=int(cfg['model'].get('iaf_hidden_dim', 128)),
    )
    ...
    res = train_structured_m2(
        ...
        lagging_inference_steps=int(cfg['training'].get('lagging_inference_steps', 0)),
        lagging_inference_joint_epochs=int(cfg['training'].get('lagging_inference_joint_epochs', 0)),
        ...
        logger=logger,
        diagnostics_dir=stage_dir,
        collect_gradient_probes=bool(log_cfg.get('save_gradient_probes', True)),
    )
    ...
    train_metrics_enabled = bool(log_cfg.get('save_train_metrics', False))
    train_predictions_enabled = bool(log_cfg.get('save_train_predictions', False))
    train_latent_enabled = bool(log_cfg.get('save_train_latent_summary', False))
    need_train_eval = bool(train_metrics_enabled or train_predictions_enabled or train_latent_enabled)
    labeled_train_eval = _make_eval_loader_from_dataset(dls.train_pretrain_labeled.dataset, batch_size=int(training_cfg.get('batch_size', 256)))
    ...
    train_out = evaluate_structured(model, labeled_train_eval, device, priors, thresholds=thresholds) if need_train_eval else None
    ...
    for ckpt_name in ['best_pretrain.pt', 'best_joint.pt', 'best_equalweight.pt', 'best_overall.pt', 'last.pt']:
        ...
        del val_ckpt, test_ckpt
        _clear_runtime_memory()
    load_checkpoint(ckpt_path, model=model)
    model.to(device)

    diagnostic_ckpts = [res.checkpoint_used]
    if res.best_joint_epoch >= 0 and 'best_joint.pt' != res.checkpoint_used and (run_dir / 'checkpoints' / 'best_joint.pt').exists():
        diagnostic_ckpts.append('best_joint.pt')

    branch_ablation_by_checkpoint = {}
    representation_summary_by_checkpoint = {}
    probe_results_by_checkpoint = {}
    if bool(log_cfg.get('save_branch_ablations', True)) or bool(log_cfg.get('save_representation_probes', True)) or bool(log_cfg.get('save_capacity_diagnostics', True)):
        for diag_ckpt in diagnostic_ckpts:
            load_checkpoint(run_dir / 'checkpoints' / diag_ckpt, model=model)
            model.to(device)
            if bool(log_cfg.get('save_branch_ablations', True)):
                branch_ablation_by_checkpoint[diag_ckpt] = _evaluate_ablation_suite(model, dls, device, priors, task_aliases, threshold_metric=str(training_cfg.get('threshold_metric', 'bal_acc')))
                _clear_runtime_memory()
            if bool(log_cfg.get('save_capacity_diagnostics', True)):
                representation_summary_by_checkpoint[diag_ckpt] = {
                    'val': summarize_classifier_representations(collect_classifier_representations(model, dls.val_all, device, classifier_ablation='full')),
                    'test': summarize_classifier_representations(collect_classifier_representations(model, dls.test_all, device, classifier_ablation='full')),
                }
                _clear_runtime_memory()
            if bool(log_cfg.get('save_representation_probes', True)):
                probe_results_by_checkpoint[diag_ckpt] = _fit_probe_suite(
                    _collect_task_probe_arrays(model, labeled_train_eval, device, task_aliases, classifier_ablation='full'),
                    _collect_task_probe_arrays(model, dls.val_all, device, task_aliases, classifier_ablation='full'),
                    _collect_task_probe_arrays(model, dls.test_all, device, task_aliases, classifier_ablation='full'),
                    seed=seed,
                )
                _clear_runtime_memory()
        load_checkpoint(ckpt_path, model=model)
    ...
    if train_predictions_enabled and train_out is not None:
    ...
    if train_out is not None and 'protox' in train_out.confusion:
    ...
    for task in task_aliases:
        ...
        outputs.extend(([train_path] if train_out is not None else []) + [val_path, test_path])
        confusion_payload[task] = {
            'train': train_out.confusion[task].tolist() if train_out is not None else None,
            'val': val_out.confusion[task].tolist(),
            'test': test_out.confusion[task].tolist(),
        }

    if bool(log_cfg.get('save_probability_arrays', True)) and train_out is not None:
    ...
    if bool(log_cfg.get('save_latent_summary', True)):
        ...
        if train_out is not None and train_latent_enabled:
            latent_payload['train'] = _latent_summary(train_out.predictions)
        write_json(latent_summary_path, latent_payload)
    ...
    if branch_ablation_by_checkpoint:
        p = stage_dir / 'branch_ablation_by_checkpoint.json'
        write_json(p, branch_ablation_by_checkpoint)
        outputs.append(p)

    if representation_summary_by_checkpoint:
        p = stage_dir / 'classifier_representation_summary_by_checkpoint.json'
        write_json(p, representation_summary_by_checkpoint)
        outputs.append(p)

    if probe_results_by_checkpoint:
        p = stage_dir / 'representation_probes_by_checkpoint.json'
        write_json(p, probe_results_by_checkpoint)
        outputs.append(p)

    change_ledger_path = stage_dir / 'change_ledger.json'
    write_json(change_ledger_path, {
        'parent_run_id': str(cfg.get('run', {}).get('parent_run_id', '2026-04-09__23-07-55__m2_3head_v13_truecount_ifm__m2_v13_truecountchirlog_tox24_seed0')),
        'feature_line_id': feature_id,
        'classifier_fp_view': str(cfg['model'].get('classifier_fp_view', 'binary_presence')),
        'encoder_fp_view': 'logcount',
        'decoder_fp_view': 'logcount',
        'ifm_enabled': bool(cfg['model'].get('ifm_num_frequencies', 0)) > 0,
        'ifm_branch': 'classifier_fp_presence',
        'ifm_include_raw': bool(cfg['model'].get('ifm_include_raw', False)),
        'classifier_use_descriptor_branch': bool(cfg['model'].get('classifier_use_descriptor_branch', True)),
        'classifier_use_logcount_branch': bool(cfg['model'].get('classifier_use_logcount_branch', False)),
        'classifier_logcount_branch_dim': cfg['model'].get('classifier_logcount_branch_dim', None),
        'iaf_num_flows': int(cfg['model'].get('iaf_num_flows', 0)),
        'iaf_hidden_dim': int(cfg['model'].get('iaf_hidden_dim', 128)),
        'clf_hidden': list(cfg['model']['clf_hidden']),
        'enc_hidden': list(cfg['model']['enc_hidden']),
        'dec_hidden': list(cfg['model']['dec_hidden']),
        'z_dim': int(cfg['model']['z_dim']),
        'joint_unlabeled_weight': float(training_cfg.get('joint_unlabeled_weight', 0.0)),
        'joint_unlabeled_batches_per_step': int(training_cfg.get('joint_unlabeled_batches_per_step', 0)),
        'beta_kl': float(training_cfg.get('beta_kl', 0.0)),
        'gen_weight': float(training_cfg.get('gen_weight', 0.0)),
        'beta_schedule': str(training_cfg.get('beta_schedule', 'warmup')),
        'lagging_inference_steps': int(training_cfg.get('lagging_inference_steps', 0)),
        'lagging_inference_joint_epochs': int(training_cfg.get('lagging_inference_joint_epochs', 0)),
        'training_regime': training_regime,
        'train_eval_scope': 'labeled_only' if need_train_eval else 'skipped',
        'run_prefix': str(cfg.get('run', {}).get('prefix', 'm2')),
    })
    outputs.append(change_ledger_path)
    ...
    eval_json = {
        ...
        'thresholds': thresholds,
        'grad_clip_hits': int(res.grad_clip_hits),
        'grad_clip_total_batches': int(res.grad_clip_total_batches),
        'grad_clip_hit_rate': float(res.grad_clip_hits / max(res.grad_clip_total_batches, 1)),
        'train_val_gap': _train_val_gap(_metrics_or_empty(train_out), val_out.metrics, task_aliases),
        'train': _metrics_or_empty(train_out),
        'val': val_out.metrics,
    ...
    write_json(summary_json, {
        ...
        'diagnostic_checkpoints': diagnostic_ckpts,
    ...
    write_manifest(
        ...
        inputs=input_paths,
        outputs=outputs,
        extra={'run_id': run_id, 'feature_id': feature_id, 'best_epoch': int(res.best_epoch), 'best_score': float(res.best_score), 'best_pretrain_epoch': int(res.best_pretrain_epoch), 'best_joint_epoch': int(res.best_joint_epoch), 'pretrain_epochs_used': int(res.pretrain_epochs_used), 'thresholds': thresholds, 'task_aliases': task_aliases, 'evaluated_checkpoints': sorted(checkpoint_eval.keys()), 'diagnostic_checkpoints': diagnostic_ckpts, 'training_regime': training_regime, 'train_label_fraction': float(training_cfg.get('train_label_fraction', 1.0)), 'train_label_mask_seed': int(training_cfg.get('train_label_mask_seed', seed)), 'grad_clip_hit_rate': float(res.grad_clip_hits / max(res.grad_clip_total_batches, 1))},
    )
